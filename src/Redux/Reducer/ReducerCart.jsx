...
const ReducerCart = (state = initalState, action) => {
  switch (action.type) {
    ...
    case "ADD_DATA":
      state = {
        id_user: action.data.user,
        listCart: action.data.cart,
      };
    ...
    case "ADD_CART":
      ...
      if (add_cart.length < 1) {
        ...
        if (!findCart) {
          ...
          add_cart[indexCart].total =
            add_cart[indexCart].total +
            parseInt(add_cart[indexCart].priceProduct) *
    ...
    case "DELETE_CART":
      ...
      console.log(state);
      // UserAPI.updateUser(state.listCart).then((data) => console.log(data));
      return state;
    ...
    case "UPDATE_CART":
      ...
      update_cart[index].total =
        data_update_cart.count * update_cart[index].priceProduct;
      state = {
        id_user: state.id_user,
